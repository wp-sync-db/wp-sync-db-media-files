...
class WPSDB_Media_Files extends WPSDB_Addon
{
    ...
    public function get_flat_attachments($attachments)
    {
    ...
    public function process_attachment_data($attachment)
    {
    ...
    public function remove_local_attachments($remote_attachments)
    {
    ...
    public function media_diff($site_a_attachments, $site_b_attachments, $site_a_media, $site_b_media)
    {
    ...
    public function add_files_to_migrate($attachment, $remote_media)
    {
        if (isset($remote_media[$attachment['file']])) {
            $this->files_to_migrate[$attachment['file']] = $remote_media[$attachment['file']];
        }
        if (empty($attachment['sizes']) || apply_filters('wpsdb_exclude_resized_media', false)) {
            return;
        }
    ...
    public function maybe_add_resized_images($attachment, $site_b_media, $site_a_media)
    {
        if (empty($attachment['sizes']) || apply_filters('wpsdb_exclude_resized_media', false)) {
            return;
        }
    ...
    public function cli_migration($outcome, $profile, $verify_connection_response, $initiate_migration_response)
    {
        ...
        if (is_wp_error($determine_media_to_migrate_response = $wpsdb_cli->verify_cli_response($response, 'ajax_determine_media_to_migrate()'))) {
        ...
        while (!empty($files_to_migrate)) {
            ...
            foreach ($files_to_migrate as $file_to_migrate => $file_size) {
                ...
                if (is_wp_error($migrate_media_response = $wpsdb_cli->verify_cli_response($response, 'ajax_migrate_media()'))) {
